...
from read_data import read_cifar10, read_mnist

from normalized_sgd import Normalized_SGD
...
def train_loop(dataloader, model, loss_fn, optimizer, device):
    ...
    for batch, sample in enumerate(pbar):
        ...
        optimizer.zero_grad()
        loss.backward()
        x_inp = model.inputs_ext(x)
        optimizer.step(x_inp)
        #optimizer.step()
...
if __name__ == '__main__':
    ...
    train_generator,test_generator = read_mnist(batchsize,data_dir)
    ...
    #optimizer = Normalized_SGD([{'params':weight_p,'weight_decay':1e-4},
    #                            {'params':bias_p,'weight_decay':0}],lr=Lr,momentum=0.9)
